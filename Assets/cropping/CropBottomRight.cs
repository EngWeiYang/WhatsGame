...
public class CropBottomRight : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        heightManager.originalBottomRightYPosCropBox = cropBoxRectTransform.localPosition.y;

        heightManager.width = cropBoxRectTransform.sizeDelta.x;
        originalXPos = handleBottomRightRectTransform.localPosition.x;
        heightManager.originalBottomRightXPosCropBox = cropBoxRectTransform.localPosition.x;
    }
    ...
    private void Update()
    {
        if (dragStarted)
        {
            cropBoxRectTransform.localPosition = new Vector3((heightManager.originalBottomRightXPosCropBox + differencex / 2), (heightManager.originalBottomRightYPosCropBox + differencey / 2), 0);
